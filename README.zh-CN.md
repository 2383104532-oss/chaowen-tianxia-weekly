...
> ## ⚠️ 依赖 dsh-worktable 运行
> **完整功能**（周报浏览页的「**设置**」「**运行记录**」标签、配置读写、运行日志查看）需在开源项目 **[dsh-worktable](https://github.com/Aisland-SJL/dsh-worktable)** —— DeepSeek Harness 的**工作台插件**（侧边栏应用抽屉 + 可停靠分屏工作区 + 每个项目的实时控制室）下运行。请将 `weekly-viewer.html` 作为工作台窗口加载；若用 `file://` 独立打开，仅能浏览周报。
