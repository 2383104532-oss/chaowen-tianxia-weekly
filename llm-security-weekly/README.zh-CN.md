...
> 脚本目录的中文说明。English version → [README.md](README.md)

每周自动采集 **GitHub AI/大模型热门项目 + 大模型安全论文 + 英文安全新闻**，
...
├── collect.ps1        # 采集：GitHub API + Crossref 论文 + HN/RSS 新闻 → raw-<时间戳>.json
├── summarize.ps1      # 摘要：调用 DeepSeek 生成中文周报 Markdown
├── send_email.ps1     # 发送：QQ SMTP 发送周报邮件（正文为 HTML）
├── run_weekly.ps1     # 总入口：采集 → 摘要 → 发邮件（含每周门控与日志）
└── README.md
```
